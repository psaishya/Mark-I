...
//     const blob = await fetchVideoStream(url);
//     const mediaStream = new MediaStream();
//     mediaStream.addTrack(blob.stream().getVideoTracks()[0]); // Assuming the blob contains video track
//     return mediaStream;
// }
...
let createPeerConnection=async(MemberId)=>{
    ...
    remoteStream=('http://localhost:5000/video_feed');
...
let toggleCamera=async()=> {
    ...
    if(videoon){
        videoon=false;
        document.getElementById('camera-btn').style.backgroundColor= 'rgb(149, 22, 22)';
        document.getElementById("user-1").src="";
